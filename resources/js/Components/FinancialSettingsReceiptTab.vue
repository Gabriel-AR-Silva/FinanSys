...
<template>
            ...
            <li v-for="forecast in forecasts.data" :key="forecast.id" class="rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
                <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                    ...
                    <div v-if="forecast.status === 'expected'" class="flex flex-wrap gap-2">
                        <button type="button" class="rounded-lg bg-emerald-700 px-3 py-2 text-sm font-semibold text-white" @click="openLink(forecast)">Vincular receita</button>
    ...
    <Modal :show="linking !== null" max-width="md" :closeable="!linkForm.processing" @close="!linkForm.processing && (linking = null)">
        <form class="space-y-4 p-5 sm:p-6" @submit.prevent="submitLink">
            <h2 class="text-lg font-semibold">Vincular receita recebida</h2>
            <p class="text-sm text-slate-500">Selecione uma receita já registrada; vinculá-la não movimenta o saldo novamente.</p>
            ...
            <p v-else class="rounded-xl bg-amber-50 p-3 text-sm text-amber-900">Não há receita livre para vincular. Registre a entrada em Lançamentos antes de voltar.</p>
            <InputError v-for="(error, field) in linkForm.errors" :key="field" :message="field === 'ledger_entry_id' ? null : error" />
            <div class="flex justify-end gap-2"><button type="button" class="rounded-xl border px-4 py-2" @click="linking = null">Voltar</button><button type="submit" :disabled="linkForm.processing || !availableReceipts.length" class="rounded-xl bg-emerald-700 px-4 py-2 font-semibold text-white disabled:opacity-50">Confirmar vínculo</button></div>
        </form>
    </Modal>
</template>
