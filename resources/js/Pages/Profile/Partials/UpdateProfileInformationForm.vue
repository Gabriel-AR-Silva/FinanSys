...
import { Camera, Trash2, UserRound } from '@lucide/vue';
import { computed, onBeforeUnmount, ref } from 'vue';

const user = usePage().props.auth.user;
const storedAvatar = user.avatar_path ? `/storage/${user.avatar_path}` : null;
const preview = ref(storedAvatar);
...
const initials = computed(() => {
    const parts = (user.name || 'Usuário').trim().split(/\s+/).filter(Boolean);
...
<template>
    <section>
        <header>
            <h2 class="text-lg font-semibold text-gray-900">Perfil do usuário</h2>
            <p class="mt-1 text-sm leading-6 text-gray-600">Personalize como você aparece no FinanSys e mantenha seus dados principais atualizados.</p>
        </header>
        ...
        <form class="mt-6 space-y-7" @submit.prevent="submit">
            <div class="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 sm:p-5">
                <div class="flex flex-col gap-5 sm:flex-row sm:items-center">
                    ...
                    <div class="min-w-0 flex-1">
                        <p class="text-sm font-semibold text-slate-900">Foto de perfil</p>
                        <p class="mt-1 text-sm leading-5 text-slate-500">Ela será usada para identificar seu usuário nas áreas do FinanSys.</p>
                        <div class="mt-3 flex flex-wrap gap-2">
                            <button type="button" class="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-3.5 py-2 text-sm font-semibold text-white transition hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2" @click="openFilePicker"><Camera :size="16" />{{ preview ? 'Trocar foto' : 'Adicionar foto' }}</button>
                            <button v-if="preview" type="button" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-sm font-semibold text-slate-600 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700" @click="removeAvatar"><Trash2 :size="16" />Remover</button>
                        </div>
                        <p class="mt-2 text-xs text-slate-500">JPG, PNG ou WebP · máximo de 2 MB.</p>
                    </div>
                </div>
                <input ref="fileInput" type="file" accept="image/jpeg,image/png,image/webp" class="sr-only" @change="chooseAvatar" />
            ...
            <div class="flex items-center gap-4 border-t border-slate-100 pt-5">
                ...
                <Transition enter-active-class="transition ease-in-out" enter-from-class="opacity-0" leave-active-class="transition ease-in-out" leave-to-class="opacity-0">
                    <p v-if="form.recentlySuccessful" class="text-sm font-medium text-emerald-700">Perfil atualizado.</p>
                </Transition>
            </div>
        </form>
    </section>
