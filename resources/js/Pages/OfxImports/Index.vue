...
watch(() => props.review, resetDrafts, { immediate: true });

const submit = () => form.post(route('ofx-imports.store'), { forceFormData: true, preserveScroll: true });
const editable = (item) => item.review_status !== 'confirmed' && !['duplicate', 'unsupported', 'card_credit_pix_candidate', 'transfer_candidate'].includes(item.classification);
...
const canConfirm = (item) => item.review_status !== 'confirmed'
    && ['income', 'expense'].includes(item.classification)
    && item.category_id
    && (item.classification !== 'expense' || item.planning_type);
const confirmSelected = () => {
    if (!props.review || selected.value.length === 0) return;

    router.post(route('ofx-imports.confirm', props.review.id), { item_ids: selected.value }, {
        preserveScroll: true,
            ...
            <section v-if="review" class="space-y-4">
                ...
                <div class="grid gap-3">
                    <article v-for="item in review.items" :key="item.id" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" :class="item.classification === 'duplicate' ? 'opacity-65' : ''">
                        <div class="grid gap-4 lg:grid-cols-[auto_1.3fr_auto_1fr] lg:items-start">
                            <label v-if="canConfirm(item)" class="flex min-h-10 items-center" :aria-label="`Selecionar ${item.description} para confirmação`">
                                <input v-model="selected" type="checkbox" :value="item.id" class="h-4 w-4 rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" />
                            </label>
                            <span v-else class="hidden lg:block" />

                            <div>
                                <div class="flex flex-wrap items-center gap-2"><span class="text-xs text-slate-500">{{ new Date(item.occurred_at).toLocaleDateString('pt-BR') }}</span><span class="rounded-full bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700">{{ label(item.classification) }}</span><span v-if="item.review_status === 'confirmed'" class="rounded-full bg-emerald-50 px-2 py-1 text-xs font-semibold text-emerald-700">Confirmado</span></div>
                            ...
                            <div v-else class="rounded-xl bg-slate-50 px-3 py-2 text-sm text-slate-600">
                                {{ item.review_status === 'confirmed' ? 'Este item já foi confirmado no financeiro.' : item.classification === 'card_credit_pix_candidate' ? 'Operação composta de cartão. Será validada em fluxo específico.' : item.classification === 'duplicate' ? 'Já identificado em importação anterior.' : 'Este item exige um fluxo específico.' }}
                            </div>
                        </div>
                    </article>
                </div>

                <div class="sticky bottom-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-lg backdrop-blur sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm text-slate-600"><strong class="text-slate-950">{{ selected.length }}</strong> item(ns) revisado(s) selecionado(s). A confirmação cria os lançamentos de forma explícita.</p>
                    <button type="button" :disabled="selected.length === 0" class="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-40" @click="confirmSelected">Confirmar selecionados</button>
                </div>

                <div class="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-950">Salvar a revisão não cria lançamento. Somente “Confirmar selecionados” envia os itens revisados ao domínio financeiro. Pix no Crédito e transferências continuam bloqueados até seus fluxos específicos.</div>
            </section>
