...
const paymentCard = computed(() => props.cards.find((card) => card.id === Number(paymentForm.credit_card_id)));
const eligibleCharges = computed(() => (paymentCard.value?.charges ?? []).filter((charge) => charge.status === 'pending' && charge.due_on.slice(0, 7) <= paymentForm.paid_on.slice(0, 7)));
...
function close() {
    if (![cardForm, purchaseForm, chargeForm, paymentForm].some((form) => form.processing)) modal.value = null;
}

function submitCharge() {
    chargeForm.transform((data) => ({ ...data, amount: normalizeMoneyInput(data.amount) }))
        .post(route('card-charges.store'), { preserveScroll: true, onSuccess: () => {
            modal.value = null;
            chargeForm.defaults({ ...chargeForm.data(), description: '', amount: '0,00', operation_id: crypto.randomUUID() });
            chargeForm.reset();
        } });
}
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex min-w-0 flex-col gap-6">
            <header class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <div class="grid grid-cols-1 gap-2 min-[420px]:grid-cols-2 lg:grid-cols-4">
                    ...
                    <button :disabled="!cards.length || !categories.length" class="rounded-xl bg-amber-600 px-3 py-2.5 text-sm font-semibold text-white disabled:opacity-40" type="button" @click="open('charge')"><ReceiptText :size="17" class="mr-1 inline" />Encargo</button>
            ...
            <section v-else class="grid min-w-0 gap-4 xl:grid-cols-2">
                <article v-for="card in cards" :key="card.id" class="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div v-if="card.charges.length" class="border-b border-slate-100 bg-amber-50/60 p-4">
                        <p class="text-xs font-semibold uppercase tracking-wide text-amber-900">Juros e multas confirmados</p>
                        <div class="mt-2 grid gap-2">
                            <div v-for="charge in card.charges" :key="charge.id" class="flex items-center justify-between gap-3 rounded-xl bg-white p-3 text-xs">
                                <span class="min-w-0"><span class="block truncate font-semibold text-slate-900">{{ charge.description }}</span><span class="text-slate-500">{{ charge.type === 'interest' ? 'Juros' : 'Multa' }} · vence {{ date(charge.due_on) }}</span></span>
                                <span class="shrink-0 text-right"><span class="block font-semibold">R$ {{ formatMoneyInput(charge.amount) }}</span><span :class="charge.status === 'paid' ? 'text-emerald-700' : 'text-amber-700'">{{ charge.status === 'paid' ? 'Pago' : `R$ ${formatMoneyInput(charge.paid_amount)} pago` }}</span></span>
                            </div>
                        </div>
                    </div>
        ...
        <Modal :show="modal === 'charge'" max-width="lg" @close="close"><form class="flex flex-col gap-4 p-5 sm:p-6" @submit.prevent="submitCharge">
            <div><h2 class="text-xl font-semibold">Confirmar juros ou multa</h2><p class="mt-1 text-sm text-slate-500">Cadastre somente um valor já confirmado pela operadora. O encargo entra como nova despesa.</p></div>
            <div class="grid gap-4 sm:grid-cols-2"><div><InputLabel for="charge-card" value="Cartão" /><select id="charge-card" v-model="chargeForm.credit_card_id" required class="mt-2 w-full rounded-xl border-slate-300"><option v-for="card in cards" :key="card.id" :value="card.id">{{ card.name }}</option></select><InputError :message="chargeForm.errors.credit_card_id" /></div><div><InputLabel for="charge-category" value="Categoria" /><select id="charge-category" v-model="chargeForm.category_id" required class="mt-2 w-full rounded-xl border-slate-300"><option disabled value="">Selecione</option><option v-for="category in categories" :key="category.id" :value="category.id">{{ category.name }}</option></select><InputError :message="chargeForm.errors.category_id" /></div></div>
            <div class="grid gap-4 sm:grid-cols-2"><div><InputLabel for="charge-type" value="Natureza" /><select id="charge-type" v-model="chargeForm.type" class="mt-2 w-full rounded-xl border-slate-300"><option value="interest">Juros</option><option value="late_fee">Multa</option></select><InputError :message="chargeForm.errors.type" /></div><div><InputLabel for="charge-kind" value="Planejamento" /><select id="charge-kind" v-model="chargeForm.planning_type" class="mt-2 w-full rounded-xl border-slate-300"><option value="ordinary">Cotidiano</option><option value="extraordinary">Extraordinário</option></select><InputError :message="chargeForm.errors.planning_type" /></div></div>
            <div><InputLabel for="charge-description" value="Descrição" /><TextInput id="charge-description" v-model="chargeForm.description" required class="mt-2 w-full" /><InputError :message="chargeForm.errors.description" /></div>
            <div class="grid gap-4 sm:grid-cols-3"><div><InputLabel for="charge-value" value="Valor confirmado" /><TextInput id="charge-value" :model-value="chargeForm.amount" inputmode="decimal" required class="mt-2 w-full" @update:model-value="chargeForm.amount = sanitizeMoneyInput($event)" /><InputError :message="chargeForm.errors.amount" /></div><div><InputLabel for="charged-on" value="Data da cobrança" /><TextInput id="charged-on" v-model="chargeForm.charged_on" type="date" :max="today" required class="mt-2 w-full" /><InputError :message="chargeForm.errors.charged_on" /></div><div><InputLabel for="charge-due" value="Vencimento" /><TextInput id="charge-due" v-model="chargeForm.due_on" type="date" required class="mt-2 w-full" /><InputError :message="chargeForm.errors.due_on" /></div></div>
            <InputError :message="chargeForm.errors.operation_id" /><div class="flex justify-end gap-2"><button type="button" class="rounded-xl border px-4 py-3 text-sm font-semibold" @click="close">Cancelar</button><button :disabled="chargeForm.processing" class="rounded-xl bg-amber-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50">Confirmar encargo</button></div>
        </form></Modal>
