    ...
    <div class="min-h-screen bg-slate-50 text-slate-950">
        <aside class="fixed inset-y-0 left-0 z-30 hidden flex-col border-r border-slate-200 bg-slate-950 py-6 text-white transition-[width,padding] duration-300 lg:flex" :class="sidebarCollapsed ? 'w-20 px-3' : 'w-72 px-5'">
            ...
            <Link :href="route('dashboard')" class="flex items-center gap-3" :class="sidebarCollapsed ? 'justify-center px-0' : 'px-2'" title="FinanSys">
                <img src="/finansys-icon.svg" alt="" width="44" height="44" class="h-11 w-11 shrink-0 rounded-2xl object-contain shadow-lg shadow-emerald-950/20" />
        ...
        <div v-if="mobileNavigationOpen" class="fixed inset-0 z-40 lg:hidden">
            ...
            <aside class="relative flex h-dvh max-h-dvh w-[min(86vw,22rem)] flex-col overflow-hidden bg-slate-950 px-5 py-5 text-white shadow-2xl" role="dialog" aria-modal="true" aria-label="Menu principal">
                <div class="flex shrink-0 items-center justify-between gap-4">
                    <Link :href="route('dashboard')" class="flex items-center gap-3" @click="closeMobileNavigation(false)"><img src="/finansys-icon.svg" alt="" width="40" height="40" class="h-10 w-10 rounded-xl object-contain" /><span class="font-semibold">FinanSys</span></Link>
                ...
                <div class="flex shrink-0 flex-col gap-3 border-t border-white/10 pt-5"><Link :href="route('profile.edit')" class="flex items-center gap-3 px-3 text-sm text-slate-300" @click="closeMobileNavigation(false)"><UserRound :size="19" />Meu perfil</Link><button type="button" class="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-rose-300 hover:bg-rose-400/10" @click="logout"><LogOut :size="19" />Sair</button></div>
            </aside>
        </div>
